...
Text::Text(GUTtext chemin, int Size, enum ColorText color)
{
    if(TTF_Init() == -1) // Initialisation de SDL_TTF pour les textes
        std::cout << "Problème lors de l'initialisation de la librairie textuelle" << TTF_GetError() << std::endl; // Message en cas d'erreur
    ...
    if(!_font) // Si il y a une erreur lors de la récupération de la police d'écriture, on envoie un message d'erreur
        std::cout << "Problème lors de l'initialisation d'une police d'écriture" << TTF_GetError() << std::endl;
...
void Text::Init(std::string text)
{
 //=========== Recherche de demandes de retours à la ligne ===========//
    ...
    foundSAVE = save.find(newline); // Recherche de \n dans la sauvegarde du texte
    ...
    else
    {
        ...
        for(int i = 0; i < COUNT; i++)
        {
            ...
            if(i < COUNT-1) // Si il y a encore des \n après, on met dans le texte la partie qu'il y a entre les deux
                text.append(save.begin() + foundSAVE + newline.length(), save.begin() + save.find(newline, foundSAVE + 1));
            else // Sinon on met dans le texte ce qu'il à apres le dernier \n
                text.append(save.begin() + foundSAVE + newline.length(), save.end());
            found = text.find(newline);
            foundSAVE = save.find(newline, foundSAVE + 1); // On cherche le prochain \n dans le texte global
            _lines.push_back(generateTexture(text.c_str())); // On met la ligne dans le tableau
        }
    }
}

GLuint Text::generateTexture(GUTtext text)
{
    SDL_Surface *_text_surf; // Surface SDL qui sera le texte

    _text_surf = TTF_RenderText_Blended(_font, text, _color); // La surface SDL devient le texte

    if(!_text_surf) // Si la surface est nulle on envoie un message d'erreur
        std::cout << "Problème lors de l'initialisation d'un texte" << SDL_GetError() << std::endl;
    ...
    {
        std::cout << "Impossible de génerer une texture à partir d'un Text" << SDL_GetError() << std::endl;
        return NULL;
    }
...
void Text::render(int x, int y, enum Alignement align)
{
     ...
     * les informations passent par la RAM avant d'êtres
